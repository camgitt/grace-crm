  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">Welcome back. Here's what needs your attention.</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Recent Visitors</h2>
            <span className="text-xs text-gray-400">Last 30 days</span>
          </div>

          {visitors.length === 0 ? (
            <p className="text-gray-400 text-sm py-8 text-center">No recent visitors</p>
          ) : (
            <div className="space-y-3">
              {visitors.slice(0, 5).map((person) => (
                ...
                >
                  ...
                  <ChevronRight size={18} className="text-gray-300 group-hover:text-gray-500 dark:group-hover:text-gray-300" />
                </button>
              ))}
            </div>
          )}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Priority Follow-Ups</h2>
            <button
              onClick={onViewTasks}
              ...
              {pendingTasks
                ...
                .map((task) => {
                  ...
                  return (
                    <div
                      key={task.id}
                      className={`p-3 rounded-xl border ${isOverdue ? 'border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/30' : 'border-gray-100 dark:border-gray-700'}`}
                    >
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <span className={`text-xs ${isOverdue ? 'text-red-600 dark:text-red-400 font-medium' : 'text-gray-400'}`}>
      ...
      {inactive.length > 0 && (
        <div className="mt-6 bg-gradient-to-r from-red-50 to-orange-50 dark:from-red-900/30 dark:to-orange-900/30 rounded-2xl p-6 border border-red-100 dark:border-red-800">
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 bg-red-100 dark:bg-red-900/50 rounded-xl flex items-center justify-center">
              <AlertCircle className="text-red-600 dark:text-red-400" size={24} />
            </div>
            <div className="flex-1">
              <h3 className="font-semibold text-gray-900 dark:text-white">Members Need Care</h3>
              ...
              <div className="flex gap-2 mt-4">
                {inactive.slice(0, 3).map((person) => (
                  <button
                    key={person.id}
                    onClick={() => onViewPerson(person.id)}
                    className="inline-flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-gray-800 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-600"
                  >
                    {person.firstName} {person.lastName}
