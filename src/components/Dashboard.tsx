              ...
              {pendingTasks
                ...
                .map((task) => {
                  ...
                  return (
                    <div
                      key={task.id}
                      className={`p-3 rounded-xl border ${isOverdue ? 'border-red-200 bg-red-50' : 'border-gray-100'}`}
                    >
      ...
      {inactive.length > 0 && (
        <div className="mt-6 bg-gradient-to-r from-red-50 to-orange-50 rounded-2xl p-6 border border-red-100">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-gray-900">Members Need Care</h3>
              ...
              <div className="flex gap-2 mt-4">
                {inactive.slice(0, 3).map((person) => (
                  <button
                    key={person.id}
                    onClick={() => onViewPerson(person.id)}
                    className="inline-flex items-center gap-2 px-3 py-1.5 bg-white rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 border border-gray-200"
                  >
                    {person.firstName} {person.lastName}
