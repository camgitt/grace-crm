    ...
    <div className="flex flex-col h-[calc(100vh-3.5rem)] max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-white dark:bg-dark-850 border-b border-gray-200/60 dark:border-white/5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 dark:text-dark-400">
            AI Assistant for {leader.displayName}
            {leader.isOnline && ' — Live Now'}
          </p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        {conversation.messages.map((message) => (
          <MessageBubble key={message.id} message={message} leader={leader} />
        ))}
      ...
      {/* Crisis Warning Footer (always visible if crisis detected) */}
  ...
  return (
    <div className={`flex items-start gap-3 ${isUser ? 'flex-row-reverse' : ''}`}>
      ...
      {isUser ? (
        ...
        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center flex-shrink-0">
          {isAI ? <Bot size={14} className="text-white" /> : (
      ...
      <div className={`max-w-[80%] ${isUser ? 'items-end' : 'items-start'}`}>
        ...
        <p className={`text-[10px] text-gray-400 dark:text-dark-500 mt-1 px-1 ${isUser ? 'text-right' : ''}`}>
          ...
          {message.sender === 'leader' && ` — ${leader.displayName}`}
        </p>
      </div>
    </div>
  );
