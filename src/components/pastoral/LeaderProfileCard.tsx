      ...
      <div className="p-5">
        <div className="flex items-start gap-4">
          ...
          <div className="relative flex-shrink-0">
            {leader.photo ? (
              ...
              <div className="w-16 h-16 rounded-full bg-gradient-to-br from-slate-500 to-slate-600 flex items-center justify-center text-white font-semibold text-xl">
                {initials}
              </div>
            )}
        ...
        <div className="flex items-center gap-2 mt-4 flex-wrap">
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 bg-slate-600 hover:bg-slate-700 text-white text-xs font-medium rounded-lg transition-colors"
          >
      ...
      {isExpanded && (
        <>
          ...
          <div className="flex border-t border-b border-gray-200 dark:border-gray-700/50">
            <button
              ...
              className={`flex-1 px-4 py-2.5 text-xs font-medium transition-colors ${
                tab === 'expertise'
                  ? 'text-slate-600 dark:text-slate-400 border-b-2 border-slate-600 dark:border-slate-400 bg-slate-50/50 dark:bg-slate-500/5'
            ...
            {tab === 'expertise' && (
              <>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {leader.expertiseAreas.map(area => (
                      <span key={area} className="text-xs px-2.5 py-1 rounded-md bg-slate-100 dark:bg-slate-500/10 text-slate-700 dark:text-slate-400">
                        {CATEGORY_LABELS[area]}
                      </span>
                    ))}
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {leader.personalityTraits.map(trait => (
                        <span key={trait} className="text-xs px-2.5 py-1 rounded-md bg-slate-100 dark:bg-slate-500/10 text-slate-700 dark:text-slate-400">
                          {trait}
                        </span>
                      ))}
                    </div>
              ...
              <>
                ...
                {leader.anchors && (
                  <div>
                    ...
                    <blockquote className="text-sm text-gray-600 dark:text-gray-400 italic border-l-2 border-slate-400 pl-3">
                      {leader.anchors}
                    </blockquote>
                  </div>
                )}
          ...
          <div className="px-5 pb-5">
            <div className="bg-gradient-to-r from-slate-600 to-slate-600 rounded-xl p-4 text-center">
              <p className="text-sm font-medium text-white mb-2">Ready to begin your spiritual journey?</p>
              <button
                onClick={() => onStartChat(leader.id)}
                className="px-5 py-2 bg-white text-slate-700 text-sm font-semibold rounded-lg hover:bg-slate-50 transition-colors"
              >
                Start Your Session
              </button>
            </div>
